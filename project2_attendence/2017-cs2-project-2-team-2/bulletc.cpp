...
Bulletc::Bulletc(QGraphicsItem *parent): QObject(),QGraphicsPixmapItem(parent){
    // set graphics
    ...
    QList<QGraphicsItem*> colliding_items=collidingItems();
        for(int i=0,n=colliding_items.size();i<n;i++){
            if(typeid(*(colliding_items[i]))==typeid(Enemy))
            {
             ...
             asEnemy->adddamage(50); // CREATE addDamage function in Enemy
             if(asEnemy->adddamage(50)==false)
